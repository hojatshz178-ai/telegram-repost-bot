...
settings = Settings(
    ...
    gemini_503_cooldown=int(os.getenv("GEMINI_COOLDOWN_503_SECONDS", "20")),
    image_search_enabled=_bool("IMAGE_SEARCH_ENABLED", True),
    wikimedia_user_agent=os.getenv(
        "WIKIMEDIA_USER_AGENT",
        "RaptorMilitaryNewsBot/1.0 (contact: admin@example.com)",
    ).strip(),
    ...
    ingest_poll_seconds=int(os.getenv("INGEST_POLL_SECONDS", "30")),
    ...
    media_retention_hours=int(os.getenv("MEDIA_RETENTION_HOURS", "6")),
    ...
    good_morning_text=os.getenv(
        "GOOD_MORNING_TEXT",
        "🌅 صبح بخیر؛ در جریان مهم‌ترین تحولات دفاعی و نظامی امروز باشید.",
    ).strip(),
    good_night_text=os.getenv(
        "GOOD_NIGHT_TEXT",
        "🌙 شب بخیر؛ مهم‌ترین تحولات نظامی و دفاعی امروز را دنبال کردید. خبرهای ضروری در ساعات سکوت نیز رصد می‌شوند.",
    ).strip(),
    admin_user_ids=tuple(int(x) for x in _csv("ADMIN_USER_IDS")),
...
def validate() -> None:
    ...
    if not settings.telegram_api_hash or settings.telegram_api_hash.startswith("REPLACE_"):
        missing.append("TELEGRAM_API_HASH")
    if not settings.telethon_session or settings.telethon_session.startswith("REPLACE_"):
    ...
    if settings.min_spacing < 30:
        raise ValueError("MIN_SPACING_MINUTES cannot be below 30")
    if missing:
